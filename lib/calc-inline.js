...
module.exports = {
  ...
  activate() {
    ...
    this.events = new CompositeDisposable(
      ...
      lumine.commands.add("lumine-workspace", {
        "calc-inline:replace": {
          description:
            "Replace the selected expression with what it evaluates to.",
          didDispatch: () => this.editorReplace(),
        },
        "calc-inline:evaluate": {
          description: "Evaluate the selected expression and show the result.",
          didDispatch: () => this.editorEvaluate(),
        },
        "calc-inline:count": {
          description: "Sum the numbers in the selection.",
          didDispatch: () => this.editorCount(),
        },
      }),
    );
  ...
  prepareRun() {
    const configuredStart = lumine.config.get("calc-inline.countStartIndex");
    ...
    if (!lumine.config.get("calc-inline.extendedVariables")) {
  ...
  getEvaluationTimeout() {
    const configuredTimeout = lumine.config.get(
      "calc-inline.evaluationTimeout",
    );
  ...
  calculateResult(expression) {
    ...
    if (lumine.config.get("calc-inline.withMath")) {
      source = `with (Math) {\n${source}\n}`;
    }
  ...
  iterateSelections(transform, { includeEmpty = false } = {}) {
    ...
    editor.getBuffer().transact(() => {
      ...
      if (
        lumine.config.get("calc-inline.evaluateAllOnEmptySelection") &&
        selections.length === 1 &&
