...
module.exports = {
  ...
  activate() {
    ...
    this.events = new CompositeDisposable(
      atom.commands.add("atom-text-editor", {
        "calc:replace": () => this.editorReplace(),
        "calc:evaluate": () => this.editorEvaluate(),
        "calc:count": () => this.editorCount(),
      }),
    );
  ...
  prepareRun() {
    const configuredStart = atom.config.get("calc.countStartIndex");
    this.selectionIndex = Number.isInteger(configuredStart) ? configuredStart : 0;
    this.resultIndex = 1;
  ...
  deleteSandboxProperties(names) {
    ...
    const serializedNames = propertyNames.map((name) => JSON.stringify(name)).join(", ");
    vm.runInContext(
  ...
  getEvaluationTimeout() {
    const configuredTimeout = atom.config.get("calc.evaluationTimeout");
  ...
  reportEvaluationError(error) {
    console.error("Unable to evaluate Calc expression", error);
    atom.notifications.addError("Calc could not evaluate an expression", {
  ...
  iterateSelections(transform, { includeEmpty = false } = {}) {
    ...
    editor.getBuffer().transact(() => {
      ...
      if (
        atom.config.get("calc.evaluateAllOnEmptySelection") &&
        selections.length === 1 &&
      ...
      const orderedSelections = [...editor.getSelections()].sort((left, right) =>
        left.compare(right),
      );
